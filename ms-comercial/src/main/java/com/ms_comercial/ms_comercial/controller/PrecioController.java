...
import com.ms_comercial.ms_comercial.assembler.PrecioModelAssembler;
...
public class PrecioController {
    ...
    @Autowired
    private PrecioModelAssembler assembler;
 
    @GetMapping
    public ResponseEntity<CollectionModel<PrecioDTO>> listarPrecio() {
        List<PrecioDTO> precios = precioService.totalPrecios();
        if (precios.isEmpty()) return ResponseEntity.noContent().build();
 
        List<PrecioDTO> preciosConLinks = precios.stream()
            .map(assembler::toModel)
            .collect(Collectors.toList());
 
        CollectionModel<PrecioDTO> collection = CollectionModel.of(
            preciosConLinks,
            linkTo(methodOn(PrecioController.class).listarPrecio()).withSelfRel()
        );
    ...
    public ResponseEntity<PrecioDTO> buscarPrecioPorId(@PathVariable Integer id) {
        try {
            PrecioDTO precio = precioService.buscarPorid(id);
            return ResponseEntity.ok(assembler.toModel(precio));
        } catch (Exception e) {
